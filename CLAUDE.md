...
- **Since the SDK 57 upgrade, `eas update` requires `--environment production`, and with that flag it ignores `.env` / `.env.production` entirely** — it bundles only the env vars stored in the EAS dashboard. Those must be populated (`eas env:create`) before the first post-upgrade OTA, or the bundle ships with no API URL / Clerk key.
- Env vars live in **two** places that must stay in sync: `eas.json` `build.production.env` (for `eas build`) and `apps/mobile/.env.production` (for `eas update` before SDK 57; now EAS-hosted env vars, see above). `EXPO_PUBLIC_POSTHOG_API_KEY` must match across `apps/mobile/.env`, `.env.production`, and `eas.json`'s `preview` + `production`.
