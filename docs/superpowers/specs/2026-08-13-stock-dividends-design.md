...
| `hooks/useFinanceActions.ts`     | 加入 dividend 的 CRUD／reinvest／summary actions。**不進 `financeStore`** —— 沿用保險模組的做法（on-demand 抓取、元件本地 state）。股利會改動 `Entry.value`，所以每次變更後呼叫既有的 `fetchAll()` 重新同步 entry；再加一份 store slice 只會多一份會與後端走味的狀態。 |
