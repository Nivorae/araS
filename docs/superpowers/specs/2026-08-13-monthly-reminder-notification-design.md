...
**狀態：已實作並實機驗證（2026-08-28，PR #121 合入 develop）。** 這是本地通知的第一版；「新功能推播」是獨立需求，
...
native rebuild 出真機版本 —— 手動把 `DAY_OF_MONTH` / `DEFAULT_HOUR` /
`DEFAULT_MINUTE` 改成幾分鐘後，驗證排程/取消/點擊 deep link 是否正常，
驗證完再改回 `1 / 9 / 0`。

**2026-08-28 實測踩到的兩個坑（不知道的話會以為功能壞了）：**

1. **改了常數再 reload 不會生效。** iOS 的重複排程排下去就歸系統管，
   `syncMonthlyReminder()` 看到「已經有一筆」就不重排，所以舊的日期時間會一直
   留著。要讓新值生效必須**重新排一次**：關開關再打開，或在畫面上改一次時間。
2. **`DEFAULT_HOUR` / `DEFAULT_MINUTE` 只是「沒存過時的預設值」。** 只要用過
   時間選擇器，AsyncStorage 裡的值就會贏過程式碼裡的常數。

因為這兩點只能靠讀出 OS 實際排程來分辨，設定頁加了一張**開發模式限定**的
「查看已排程通知」卡片（`describeScheduledReminders()`），會列出每一筆排程的
trigger 與下次觸發時間。

已驗證：通知在**背景與完全滑掉**的狀態都會送達（排程交給 iOS 通知中心，不歸
App 管）。唯一還沒驗的是「點通知回首頁」—— Expo Go 裡通知掛在 Expo Go 名下，
完全滑掉時點擊只會開 Expo Go 首頁，要等 TestFlight／正式版。

## 明確不做的事（YAGNI）

- 不做「新功能推播」（remote push）—— 需要後端存 token、需要發送機制，
  是完全不同的架構，獨立列為未來項目。
- ~~不做時間選擇器，時間固定 9:00~~ —— **2026-08-28 推翻**：實機看過之後決定
  開放時間（分鐘以 5 為級距），因為 9:00 對上班族與夜貓子的意義差很多。日期
  仍然固定每月 1 號：「回來記上個月的帳」只有月初有意義，開放選日期只是多一個
  沒有正確答案的決定。選擇器是純 JS 的 `components/TimePickerModal.tsx`
  （DatePickerModal 的雙胞胎），沒有新的原生依賴。
